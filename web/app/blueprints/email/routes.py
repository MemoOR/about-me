...
def send_email():
    ...
    try:
        send_mail(subject=f'Mensaje de mi pagina web: {user_email}',
                        sender=current_app.config['MAIL_DEFAULT_SENDER'],
                        recipients=["memo.or99@hotmail.com"],
                        text_body=f"Mensaje de: {user_email}",
                        html_body=f'<h1>{user_name}</h1><br><h2>{user_email}</h2><br><br><p>{user_message}</p>')

        response = {'text': 'I got your message and will contact you soon!', 'type':"success"}
